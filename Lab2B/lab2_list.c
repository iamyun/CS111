...
void* thread_routine(void *arg) {
	...
	int start = tid*itNum;
	int end = (tid+1)*itNum;
	struct timespec start_time_mutex;
	struct timespec end_time_mutex;
	struct timespec start_time_spin;
	struct timespec end_time_spin;
	long lns = 0;
	...
	for (i = start; i < end; i++) {
		if (sync_opt == 0)
			...
			switch(sync_opt) 
			{
				case 'm':
				{
					if (clock_gettime(CLOCK_MONOTONIC, &start_time_mutex) == -1)
					{
						fprintf(stderr, "clock mutex start time %s\n", strerror(errno));
						exit(1);
					}

					pthread_mutex_lock(&list_mutex);

					if (clock_gettime(CLOCK_MONOTONIC, &end_time_mutex) == -1)
					{
						fprintf(stderr, "clock mutex end time %s\n", strerror(errno));
						exit(1);
					}

					lns = end_time_mutex.tv_sec - start_time_mutex.tv_sec;
					lns *= 1000000000;	//convert the seconds part into nanoseconds
					lns += end_time_mutex.tv_nsec;
					lns -= start_time_mutex.tv_nsec;
					lockAqTime += lns;
				...
				case 's':
				{
					if (clock_gettime(CLOCK_MONOTONIC, &start_time_spin) == -1)
					{
						fprintf(stderr, "clock spin-lock start time %s\n", strerror(errno));
						exit(1);
					}

					while(__sync_lock_test_and_set(&lock, 1));

					if (clock_gettime(CLOCK_MONOTONIC, &end_time_spin) == -1)
					{
						fprintf(stderr, "clock spin end time %s\n", strerror(errno));
						exit(1);
					}

					lns = end_time_spin.tv_sec - start_time_spin.tv_sec;
					lns *= 1000000000;	//convert the seconds part into nanoseconds
					lns += end_time_spin.tv_nsec;
					lns -= start_time_spin.tv_nsec;
					lockAqTime += lns;
...
int main(int argc, char **argv) {
	...
	int j;
	for (j = 0; j < listNum; j++) {
		/*list[j] = malloc(sizeof(SortedList_t));
		if (list[j] == NULL) {
			fprintf(stderr, "list header malloc %s\n", strerror(errno));
			exit(1);
		}*/
		list[j].prev = &list[j];
		list[j].next = &list[j];
	...
	/*long long totalAqTime = 0;
	for (t = 0; t < threadNum; t++) {
		totalAqTime += lockAqTime[t];
	}*/

	//print to stdout a CSV
	long long opt_perform = threadNum*itNum*3;
	long long avgWait = lockAqTime/opt_perform;
	printf("%s,%d,%d,%d,%lld,%lld,%lld,%lld\n", testopt, threadNum, itNum, listNum, opt_perform, ns, ns/opt_perform, avgWait);
	exit(0);
