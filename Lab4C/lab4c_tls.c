...
void getTemp()
{
	...
	if (offFlag == 1) {
		...
		if (lFlag == 1) {
			...
			if (SSL_write(sslClient, &print, length) < 0) {
				fprintf(stderr, "Error: Failed SSL write to socket\n");
				exitNow(1);
			}
		}
		exitNow(0);
...
int main(int argc, char **argv) {
	...
	if (argc < 4) {
		fprintf(stderr, "Invalid argument. Usage: ./lab4c_tls --id=# --host=hostName --log=filename portNumber\n");
		exitNow(1);
	}
	...
	if (argc - argc_count == 1) {
		...
		fprintf(stderr, "%d %d\n", argc, argc_count);
		fprintf(stderr, "PortNumber. Invalid argument. Usage: ./lab4c_tls --id=# --host=hostName --log=filename portNumber\n");
		exitNow(1);
	}
	...
	//create SSL_CTX object to establsih TLS/SSL connections later
	SSL_CTX *sslContext = SSL_CTX_new(TLSv1_client_method());
	if (sslContext == NULL) {
		fprintf(stderr, "ERROR: Failed getting SSL Context\n");
		exitNow(2);
	}
	//create new SSL structure
	sslClient = SSL_new(sslContext);
	if (sslClient == NULL) {
		fprintf(stderr, "EEROR: Failed getting SSL structure\n");
		exitNow(2);
	}

	//fprintf(stderr, "context\n");
	//connect socket file descriptor with ssl
	if (SSL_set_fd(sslClient, sockfd) == 0) {
		fprintf(stderr, "ERROR: Failed setting up SSL file descriptor.\n");
		exitNow(2);
	}

	//fprintf(stderr, "set_fd %d\n", sockfd);
	//tls/ssl handshake/conection
	...
	if (SSL_write(sslClient, &id_buff, len) < 0) {
		fprintf(stderr, "Error: Failed ID SSL write to socket\n");
		exitNow(2);
	}
